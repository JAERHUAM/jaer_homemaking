...
if ($write_type === 'pclist') {
    ...
    if ($parent_wr_id > 0) {
        $redirect_url = short_url_clean(G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table . '&wr_id=' . $parent_wr_id . '&sublist=scena');
    } else {
        $redirect_url = short_url_clean(G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table . '&sublist=scena');
    }
} else {
    if ($parent_wr_id > 0) {
