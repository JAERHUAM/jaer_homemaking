...
$allowed_write_types = array('scena');
$current_write_type = '';
...
// 시나리오 전용 게시물은 커스텀 뷰로 이동하도록 리디렉션 주소를 필터링
if (!function_exists('tr_log_write_update_move_url')) {
    function tr_log_write_update_move_url($redirect_url, $board, $wr_id, $w, $qstr, $file_upload_msg)
    {
        ...
        // 부모 게시글 최초 작성 후에는 목록으로 이동
        if ($w === '' && !$write_type) {
            if ($current_write === null && isset($board['bo_table'])) {
                $write_table = $g5['write_prefix'] . $board['bo_table'];
                $current_write = get_write($write_table, $wr_id);
            }
            if ($current_write && isset($current_write['wr_parent']) && (int)$current_write['wr_parent'] === (int)$wr_id) {
                $custom_url = G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table;
                return short_url_clean($custom_url);
            }
        }
