...
<div id="bo_list" style="width:90%;">
    ...
    <div class="log20_list_area">
        <?php
        // 부모 게시글 수만 계산 (wr_4가 'scena', 'pclist', 'tarae'가 아닌 게시물만)
        if (!isset($write_table)) {
            $write_table = $g5['write_prefix'] . $bo_table;
        }
        $parent_count_sql = "SELECT COUNT(*) as cnt FROM {$write_table} WHERE wr_is_comment = 0 AND (wr_4 IS NULL OR wr_4 = '' OR (wr_4 != 'scena' AND wr_4 != 'pclist' AND wr_4 != 'tarae'))";
        ...
        foreach ($list as $item) {
            ...
            // wr_4가 'scena', 'pclist', 'tarae'가 아닌 게시물만 표시 (부모 게시물만)
            if ($item_wr_4 !== 'scena' && $item_wr_4 !== 'pclist' && $item_wr_4 !== 'tarae') {
