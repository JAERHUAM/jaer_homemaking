...
// 부모 게시물 ID가 있고 색상 정보가 없으면 부모 게시물에서 직접 조회
...
$back_view_url = '';
if ($parent_wr_id > 0) {
    $back_view_url = G5_BBS_URL . '/board.php?bo_table=' . $current_bo_table . '&wr_id=' . $parent_wr_id;
} else {
...
<div class="log_content_sublist_scena_title"<?php if ($scena_title_style) { ?> style="<?php echo trim($scena_title_style); ?>"<?php } ?>>
    <span class="log_content_sublist_scena_title_text" style="color: <?php echo $accent_color ? htmlspecialchars($accent_color) : '#fff'; ?>;">시나리오 목록</span>
</div>
